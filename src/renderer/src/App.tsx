...
import { ImageElement, KeyframeProperties } from './lib/scene'
...
const resizeInnerToFitOuter = (outer: any, inner: any) => {
  ...
  const scaleFactor = Math.min(outerWidth / innerWidth, outerHeight / innerHeight)
...
const AnimationPlaybar = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full p-2">
      ...
      <span className="text-xs mt-1">{millisecondsToDisplayTime(currentTime)}</span>
    </div>
  )
}
...
const Timeline = () => {
  ...
  const [, drop] = useDrop(() => ({
    ...
    drop: (item: any) => {
      ...
      new_asset.addKeyFrame(2000, k[4])
      new_asset.scale = 1
  ...
  return (
    <div ref={drop} className="block w-full h-full bg-muted overflow-auto relative">
      <div className="w-[calc(100%-40px)] pt-5 relative h-full mx-auto">
        <div className="w-full">
          {project.scene.children &&
            project.scene.children.map((e) => <ElementLifecycle element={e} key={e.id} />)}
        </div>
      </div>
    </div>
  )
